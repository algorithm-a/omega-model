...
class OmegaConfig:
    ...
    # --- v12.3 ---
    top_k: int = 8
    use_topk_obs: bool = True
    use_lags: bool = True
    use_extended_state: bool = True
    reward_v2_max_local: float = 2.0
    reward_v2_std_local: float = 1.0
    reward_v2_alive_penalty: float = 0.1
    lr_start: float = 3e-4
    lr_end: float = 1e-5
    use_lstm: bool = False
...
class OmegaUniverse:
    ...
    def complexity(self) -> float:
        """K — разнообразие ветвей: 1 - mean_pairwise_cosine."""
        if len(self.branches) < 2:
            return 0.0
        return float(np.clip(1.0 - self.mean_pairwise_cosine(), 0.0, 1.0))

    def information(self) -> float:
        """I — суммарная энергия, нормированная на max_branches."""
        if not self.branches:
            return 0.0
        total_e = float(sum(b.energy for b in self.branches.values()))
        return float(np.clip(total_e / max(1, self.config.max_branches), 0.0, 1.0))

    def omega_distance(self) -> float:
        """D — расстояние до Omega-точки: 1 - C."""
        states = (
    ...
    def top_k_branches(self, k: int) -> list["Branch"]:
        """Top-k ветвей по энергии."""
        if not self.branches:
            return []
        items = sorted(
            self.branches.values(), key=lambda b: b.energy, reverse=True
        )
...
class OmegaV12Env(gym.Env):
    ...
    def __init__(
        ...
        if reward_mode not in {"coherence", "survival", "combined", "coherence_v2"}:
            raise ValueError(
                "reward_mode must be 'coherence', 'survival', 'combined', or 'coherence_v2'"
            )
        ...
        base_dim = 7 if observation_mode == "local" else 13
        extra_dim = 0
        if observation_mode == "global":
            if self.config.use_extended_state:
                extra_dim += 3
            if self.config.use_topk_obs:
                extra_dim += self.config.top_k * STATE_DIM
        ...
        self._c_history: list[float] = [0.0, 0.0, 0.0]
    ...
    def _obs(self) -> np.ndarray:
        ...
        parts = [
            np.asarray(
                [
                    ...
                    mean_energy, std_energy, mean_local_c,
                ],
                dtype=np.float32,
            ),
            root,
        ...
        if self.config.use_extended_state:
            parts.append(
                np.asarray(
                    [
                        self.world.complexity(),
                        self.world.information(),
                        self.world.omega_distance(),
                    ],
                    dtype=np.float32,
                )
            )
        ...
        if self.config.use_topk_obs:
            top = self.world.top_k_branches(self.config.top_k)
            k = self.config.top_k
            buf = np.zeros((k, STATE_DIM), dtype=np.float32)
            for i, br in enumerate(top[:k]):
                buf[i] = br.state.astype(np.float32)
            parts.append(buf.flatten())

        if self.config.use_lags:
            parts.append(
                np.asarray(self._c_history[:3], dtype=np.float32)
            )
    ...
    def reset(self, *, seed: Optional[int] = None, options=None):
        ...
        self._c_history = [self.prev_coherence, 0.0, 0.0]
    ...
    def step(self, action):
        ...
        if self.reward_mode == "coherence_v2":
            local_cs = np.asarray(
                [b.coherence for b in self.world.branches.values()],
                dtype=np.float64,
            ) if self.world.branches else np.asarray([0.0])
            max_local_c = float(local_cs.max())
            std_local_c = float(local_cs.std())
            alive_ratio = metrics.alive / max(1, self.config.max_branches)
            reward = (
                self.config.reward_coherence * metrics.coherence
                + self.config.reward_v2_max_local * max_local_c
                + self.config.reward_v2_std_local * std_local_c
                - self.config.reward_v2_alive_penalty * alive_ratio
            )
        elif self.reward_mode == "coherence":
...
def train_ppo(
    ...
    lr_schedule = lambda progress_remaining: (
        lr_end + (lr_start - lr_end) * progress_remaining
    )
    ...
    if checkpoint_dir is not None:
        ...
        callbacks.append(
            CheckpointCallback(
                save_freq=max(1, int(checkpoint_every)),
                save_path=str(checkpoint_dir),
                name_prefix=f"ppo_s{seed}",
...
def run_condition(
    ...
    if algorithm == "random":
        ...
        ckpt_dir = out_dir / f"ckpt_{complexity}_b{config.initial_branches}_s{seed}"
        policy = train_ppo(
            env, seed, train_steps,
            save_path=save_path,
...
def run_experiment(
    ...
    base_cfg.use_lstm = use_lstm
    base_cfg.top_k = top_k
    base_cfg.use_topk_obs = use_topk
    base_cfg.use_lags = use_lags
    base_cfg.use_extended_state = use_extended
    rows = []
...
def parse_args():
    ...
    p.add_argument(
        "--reward",
        choices=["coherence", "survival", "combined", "coherence_v2"],
        default="combined",
    )
    p.add_argument("--policy", choices=["mlp", "lstm"], default="mlp")
    p.add_argument("--top-k", type=int, default=8)
    p.add_argument("--no-topk", action="store_true")
...
def main():
    ...
    if args.mode == "train":
        rows = run_experiment(
            ...
            use_lstm=(args.policy == "lstm"),
            top_k=args.top_k,
            use_topk=not args.no_topk,
