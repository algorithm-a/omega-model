"""
omega_branch_action.py
Версия Omega-модели с RecurrentPPO, reward 'growth' и ВЕКТОРНЫМ действием.

Отличия от omega_growth.py:
- action_space = (3,) вместо (1,) — по одному theta на каждую глубину
- micro_action_vector: theta[0] для depth=0, theta[1] для depth=1, theta[2] для depth>=2
- Агент получает в 3 раза больше рычага
- OUTDIR = experiment_output_branch_action

Запуск:
    python omega_branch_action.py
"""
...
class OmegaUniverseEngine:
    ...
    def step_energy(self) -> None:
        """Обновление энергии ветвей."""
        for b in list(self.multiversal_sphere.values()):
            b.age += 1
            b.energy -= 0.0005
            b.energy += 0.3 * b.coherence
            b.energy = min(b.energy, 1.0)

    def prune_branches(self) -> int:
        """Удаление мёртвых ветвей. Возвращает число удалённых."""
        # Не трогаем root и ветви с малой энергией (только что созданные)
        if len(self.multiversal_sphere) <= 1:
            return 0
        alive = {}
        for bid, b in self.multiversal_sphere.items():
            # root всегда остаётся
            if bid == "root":
                alive[bid] = b
                continue
            # смерть по энергии или по когерентности (только если coherence > 0)
            if b.energy <= 0:
                continue
            if b.coherence > 0 and b.coherence < 0.001:
                continue
            alive[bid] = b
        removed = len(self.multiversal_sphere) - len(alive)
        self.multiversal_sphere = alive
        return removed

    def spawn_branches(self, parent_id: str, n: int = 1) -> int:
        """Создание новых ветвей. Возвращает число созданных."""
        parent = self.multiversal_sphere.get(parent_id)
        if parent is None or parent.energy < 0.1:
            return 0
        created = 0
        for i in range(n):
            parent.energy -= 0.1
            new_id = f"{parent_id}.{len(self.multiversal_sphere)}"
            new_branch = Branch(
                id=new_id,
                state_vector=parent.state_vector,
                depth=parent.depth + 1,
                parent=parent_id,
                energy=0.5,
            )
            self.multiversal_sphere[new_id] = new_branch
            self.simulate_experience(new_id)
            created += 1
        return created
    ...
    def micro_action(self, action: float) -> None:
        """Скалярное воздействие (оставлено для совместимости)."""
    ...
    def run_cycle(
        ...
        for cycle in range(max_cycles):
            ...
            if collapsed and False:  # ОТКЛЮЧЕНО для эволюции
                if verbose:
...
def all_metrics(
    ...
    return {
        ...
        "mean_C": float(np.mean(coherence_traj)) if coherence_traj else 0.0,
        "final_S": float(entropy_traj[-1]) if entropy_traj else 0.0,
        "mean_S": float(np.mean(entropy_traj)) if entropy_traj else 0.0,
    }
...
class OmegaLifeEnv(gym.Env):
    ...
    def __init__(
        self,
        reward_mode: str = "growth",
        initial_seed: int = 42,
        action_mode: str = "vector",
        episode_length: int = EPISODE_LENGTH,
    ):
        super().__init__()
        if reward_mode not in {"order", "life", "growth", "evolution"}:
            raise ValueError(
                "reward_mode должен быть 'order', 'life' или 'growth'"
            )
    ...
    def step(self, action):
        ...
        if self.action_mode == "scalar":
            ...
            action_vec = np.clip(action[:ACTION_DIM], -1.0, 1.0)
            self.engine.micro_action_vector(action_vec)
            self.engine.run_cycle(
                max_cycles=1, verbose=False, enhance_on_no_omega=False
            )

        # ЭВОЛЮЦИЯ: обновление энергии и удаление мёртвых ветвей
        self.engine.step_energy()
        removed = self.engine.prune_branches()
        if len(self.engine.multiversal_sphere) < 40:
            self.engine.spawn_branches("root", n=2)

        new_coherence = self.engine.global_coherence()
        new_entropy = self.engine.shannon_entropy()
        if self.steps % 20 == 0:
            print(f"  [DBG step={self.steps}] branches={len(self.engine.multiversal_sphere)}, C={new_coherence:.4f}, S={new_entropy:.4f}")

        if self.reward_mode == "order":
            reward = new_coherence * 10.0 if new_coherence > 0.1 else 0.0
        elif self.reward_mode == "growth":
            reward = new_coherence - 0.1 * abs(new_entropy - TARGET_S)
        elif self.reward_mode == "evolution":
            alive = len(self.engine.multiversal_sphere)
            initial = 40
            reward = (
                0.5 * (alive / initial)
                + 0.3 * new_coherence
                - 0.2 * new_entropy / 12.0
            )
        else:
            reward = new_coherence * 10.0 + new_entropy / 20.0 if new_coherence > 0.1 else 0.0
...
class OmegaLifeEnvControlled(OmegaLifeEnv):
    ...
    def step(self, action):
        ...
        if self.shuffle_reward:
            reward = float(np.random.uniform(0.0, 1.0))
...
CONDITIONS = [
    ("random_evolution", "evolution", "random", {}),
    ("trained_evolution", "evolution", "trained", {}),
    ("shuffled_evolution", "evolution", "trained", {"shuffle_reward": True}),
    ("constant_reward", "evolution", "trained", {"no_step": True}),
    ("mcts_evolution", "evolution", "mcts", {"n_simulations": 3}),
]
...
def make_mcts_policy(env_factory, n_simulations=20):
    """MCTS-политика для OmegaLifeEnvControlled."""
    def mcts_policy(obs):
        sim_env = env_factory()
...
def make_policy(
    ...
    if policy_type == "mcts":
        n_sim = controls.get("n_simulations", 20)
        return make_mcts_policy(env_factory, n_simulations=n_sim)
...
def run_single(
    ...
    policy = make_policy(env_factory, policy_type, seed, controls)
...
def main():
    print("=" * 72)
    print("ОМЕГА-ЭКСПЕРИМЕНТ (v6: векторное действие)")
    print("=" * 72)
    if PROFILE_FIRST_SEED:
        profile_single_episode()
    suggested = calibrate_omega_threshold()
    # global OMEGA_THRESHOLD
    # OMEGA_THRESHOLD = suggested   # НЕ перезаписываем!
    print(f"[CFG] suggested_threshold = {suggested:.4f}, keeping OMEGA_THRESHOLD = {OMEGA_THRESHOLD:.4f}")
    rows = []
    trajectories = {}
    for condition, mode, policy, controls in CONDITIONS:
        ...
        for seed in SEEDS:
            ...
            print(
                ...
                f"C={metrics['final_C']:.4f}, mC={metrics['mean_C']:.4f}, "
    ...
    print("=" * 72)
    test_pairs = [
        ("trained_evolution", "random_evolution"),
        ("trained_evolution", "shuffled_evolution"),
        ("trained_evolution", "constant_reward"),
    ]
    test_rows = []
