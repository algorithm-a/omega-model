"""
omega_unified.py
Рабочий прототип Omega-модели.

Патч v3:
- micro_action: направленное смещение (copysign, bias=0.7)
- run_cycle: big_bang только если сфера пуста
- coherence_collapse: после коллапса сразу big_bang
- TRAIN_STEPS = 50000
...
class OmegaLifeEnvControlled(OmegaLifeEnv):
    ...
    def step(self, action):
        ...
        if self.shuffle_reward:
            # Полная замена reward на случайный — сильный контроль
...
def main():
    ...
    # OMEGA_THRESHOLD = suggested  # НЕ перезаписываем глобальную
    print(f"[CFG] suggested_threshold = {suggested:.4f}, "
          f"keeping OMEGA_THRESHOLD = {OMEGA_THRESHOLD:.4f}")
    rows = []
    trajectories = {}
