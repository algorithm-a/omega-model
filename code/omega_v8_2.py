"""
omega_v8_2.py
Версия Omega-модели v8.2: рост среды только при C > 0.05.

Отличия от omega_v8_1.py:
- apply_parameters_action принимает current_C
- branching_factor растёт только при C > 0.05
- max_depth растёт только при C > 0.07
- Уменьшение branching и depth ЗАПРЕЩЕНО
- OUTDIR = experiment_output_v8_2

Запуск:
    python omega_v8_2.py
"""
...
OUTDIR = "experiment_output_growth_v2"
...
# Reward коэффициенты (ослабленные, как в v8.1)
REWARD_THRESHOLD_C = 0.05
PENALTY_BELOW_THRESHOLD = 1.0
REWARD_SCALE_ABOVE = 2.0
OMEGA_PENALTY_SCALE = 1.0
OMEGA_BONUS_SCALE = 2.0
OMEGA_BONUS_STRONG = 3.0
DEAD_ENV_PENALTY = 2.0

# Пороги для роста среды
BRANCHING_GROWTH_THRESHOLD_C = 0.05
DEPTH_GROWTH_THRESHOLD_C = 0.07

MICRO_DIM = 3
PARAMS_DIM = 3
ACTION_DIM = MICRO_DIM + PARAMS_DIM
...
class OmegaUniverseEngine:
    ...
    def apply_parameters_action(
        self,
        params_action: np.ndarray,
        current_C: float,
    ) -> None:
        """
        Управление параметрами среды с привязкой к прогрессу C.

        branching_factor растёт только при C > BRANCHING_GROWTH_THRESHOLD_C (0.05)
        max_depth растёт только при C > DEPTH_GROWTH_THRESHOLD_C (0.07)
        noise: свободно

        Уменьшение branching и depth ЗАПРЕЩЕНО.
        """
        # branching_factor: только рост, и только при C > 0.05
        if (
            params_action[0] > 0.3
            and current_C > BRANCHING_GROWTH_THRESHOLD_C
        ):
            self.branching_factor = min(
                self.branching_factor + 1, MAX_BRANCHING_FACTOR
            )
        ...
        # max_depth: только рост, и только при C > 0.07
        if (
            params_action[1] > 0.3
            and current_C > DEPTH_GROWTH_THRESHOLD_C
        ):
            self.max_depth = min(self.max_depth + 1, MAX_DEPTH_HARD)
        # уменьшения НЕТ

        # noise: свободно
        if params_action[2] > 0.3:
            self.noise = min(self.noise * 1.5, 0.1)
        elif params_action[2] < -0.3:
            self.noise = max(self.noise * 0.7, 0.005)

    def micro_action_vector(self, action_vec: np.ndarray) -> None:
        """Векторное воздействие: theta[i] действует на ветви глубины i."""
        n_depths = len(action_vec)
        for branch in self.multiversal_sphere.values():
            depth_idx = min(branch.depth, n_depths - 1)
            theta = float(np.clip(action_vec[depth_idx], -1.0, 1.0))
            sv = np.asarray(branch.state_vector, dtype=float)
            mean_sv = float(sv.mean())
            bias = theta * 0.7 * math.copysign(1.0, mean_sv)
            distorted = sv + bias
    ...
    def micro_action(self, action: float) -> None:
        """Скалярное воздействие (для совместимости)."""
...
class OmegaLifeEnv(gym.Env):
    ...
    def __init__(
        self,
        reward_mode: str = "omega",
        initial_seed: int = 42,
        action_mode: str = "scalar",
        episode_length: int = EPISODE_LENGTH,
    ):
        super().__init__()
        if reward_mode not in {"order", "life", "growth", "omega"}:
            raise ValueError(
                "reward_mode должен быть 'order', 'life', 'growth' или 'omega'"
            )
        if action_mode not in {"scalar", "vector", "params"}:
            raise ValueError(
                "action_mode должен быть 'scalar', 'vector' или 'params'"
            )
        ...
        if action_mode == "scalar":
            ...
            self.action_space = spaces.Box(
                low=-1.0, high=1.0, shape=(MICRO_DIM,), dtype=np.float32
            )
        else:  # params
            self.action_space = spaces.Box(
    ...
    def _compute_reward(self, new_C: float, new_S: float) -> float:
        """Мягкий reward режим 'omega'."""
        # 1. Основной reward: награда только выше REWARD_THRESHOLD_C
        if new_C > REWARD_THRESHOLD_C:
            reward = REWARD_SCALE_ABOVE * (new_C - REWARD_THRESHOLD_C)
        else:
            reward = -PENALTY_BELOW_THRESHOLD * (
                REWARD_THRESHOLD_C - new_C
            ) / REWARD_THRESHOLD_C

        # 2. Штраф за отклонение S
        reward -= 0.1 * (new_S - TARGET_S) ** 2

        # 3. Штраф за мёртвую среду
        if new_C < 0.001:
            reward -= DEAD_ENV_PENALTY

        # 4. Малые бонусы за рост среды
        if self.engine is not None:
            if self.engine.branching_factor >= 5:
                reward += 0.2
            if self.engine.max_depth >= 5:
                reward += 0.2
            if self.engine.noise <= 0.005:
                reward += 0.2

        # 5. Omega-функция: штраф далеко + бонус близко
        progress = min(new_C / OMEGA_THRESHOLD, 2.0)
        reward += -OMEGA_PENALTY_SCALE * ((1.0 - progress) ** 2)
        reward += OMEGA_BONUS_SCALE * (progress ** 2)
        if progress > 0.7:
            reward += OMEGA_BONUS_STRONG * (progress ** 2)
    ...
    def step(self, action):
        ...
        if self.action_mode == "scalar":
            ...
            micro_vec = np.clip(action[:MICRO_DIM], -1.0, 1.0)
            self.engine.micro_action_vector(micro_vec)
        else:  # params
            micro_vec = np.clip(action[:MICRO_DIM], -1.0, 1.0)
            params_vec = np.clip(
                action[MICRO_DIM:MICRO_DIM + PARAMS_DIM], -1.0, 1.0
            )

            # Сначала micro_action
            self.engine.micro_action_vector(micro_vec)

            # Текущее C — до run_cycle
            current_C = self.engine.global_coherence()

            # Затем параметры с учётом current_C
            self.engine.apply_parameters_action(params_vec, current_C)
        ...
        if self.reward_mode == "order":
            ...
            reward = new_coherence - 0.1 * abs(new_entropy - TARGET_S)
        elif self.reward_mode == "omega":
            reward = self._compute_reward(new_coherence, new_entropy)
        else:  # life
            reward = (
        ...
        return (
            ...
            truncated,
            {
                ...
                "entropy": new_entropy,
                "branching_factor": self.engine.branching_factor,
                "max_depth": self.engine.max_depth,
                "noise": self.engine.noise,
            },
        )
    ...
    def step(self, action):
        ...
        if self.freeze_action:
            if self.action_mode == "scalar":
                ...
                action = np.zeros(MICRO_DIM, dtype=np.float32)
            else:
...
CONDITIONS = [
    ("frozen_computed", "growth", "frozen", {"freeze_action": True}),
    ("random_growth", "growth", "random", {}),
    ("trained_growth", "growth", "trained", {}),
    ("constant_reward", "growth", "trained", {"no_step": True}),
]
...
def run_single(
    ...
    branching_traj, depth_traj, noise_traj = [], [], []
    total_reward = 0.0
    ...
    for _ in range(EPISODE_LENGTH):
        ...
        branching_traj.append(int(info["branching_factor"]))
        depth_traj.append(int(info["max_depth"]))
        noise_traj.append(float(info["noise"]))
    ...
    metrics.update(
        {
            ...
            "final_branching": (
                int(branching_traj[-1]) if branching_traj else 0
            ),
            "final_depth": (
                int(depth_traj[-1]) if depth_traj else 0
            ),
            "final_noise": (
                float(noise_traj[-1]) if noise_traj else 0.0
            ),
        }
    )
    return {
        ...
        "branching": np.asarray(branching_traj, dtype=np.int32),
        "depth": np.asarray(depth_traj, dtype=np.int32),
        "noise": np.asarray(noise_traj, dtype=np.float32),
    }
...
def main():
    print("=" * 72)
    print("ОМЕГА-ЭКСПЕРИМЕНТ (v8.2: рост среды при C > 0.05)")
    print("=" * 72)
    ...
    print(f"[CFG] DEPTH_GROWTH_THRESHOLD_C = {DEPTH_GROWTH_THRESHOLD_C}")
    rows = []
    trajectories = {}
    for condition, mode, policy, controls in CONDITIONS:
        ...
        for seed in SEEDS:
            ...
            print(
                ...
                f"branching={metrics['final_branching']}, "
                f"depth={metrics['final_depth']}, "
                f"noise={metrics['final_noise']:.4f}, "
    ...
    print("=" * 72)
    test_pairs = [
        ("trained_growth", "random_growth"),
        ("trained_growth", "frozen_computed"),
        ("trained_growth", "constant_reward"),
    ]
    test_rows = []
