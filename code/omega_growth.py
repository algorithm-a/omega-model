"""
omega_growth.py
Версия Omega-модели с RecurrentPPO и reward, стимулирующим РОСТ когерентности.

Отличия от omega_recurrent.py:
- Reward-функция life изменена: вместо -|C-0.3|-0.1|S-6| стало +C-0.1|S-6|
- Агент поощряется за рост C, а не за удержание точки 0.3
- Результаты сохраняются в experiment_output_growth/

Запуск:
    python omega_growth.py
...
OUTDIR = "experiment_output_growth"
...
class OmegaLifeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.reward_mode == "order":
            ...
            # НОВЫЙ РЕЖИМ: поощряем рост C, удерживаем S около 6.0
...
def main():
    print("=" * 72)
    print("ОМЕГА-ЭКСПЕРИМЕНТ (v5: рост C)")
    print("=" * 72)
