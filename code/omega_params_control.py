"""
omega_params_control.py
Версия Omega-модели с RecurrentPPO, reward 'growth' и управлением ПАРАМЕТРАМИ среды.

Отличия от omega_branch_action.py:
- action_space = (6,) вместо (3,)
- action[0:3] — micro_action (как в v6)
- action[3:6] — управление параметрами:
    action[3] -> branching_factor
    action[4] -> max_depth
    action[5] -> noise
- RL может сам усиливать среду (аналог enhance_simulation, но под контролем)
- OUTDIR = experiment_output_params_control

Запуск:
    python omega_params_control.py
"""
...
class OmegaUniverseEngine:
    ...
    def apply_parameters_action(self, params_action: np.ndarray) -> None:
        """
        Управление параметрами среды через действие агента.

        params_action[0] — branching_factor:
            > 0.3  → +1
            < -0.3 → -1
            иначе  → без изменений

        params_action[1] — max_depth:
            > 0.3  → +1
            < -0.3 → -1

        params_action[2] — noise:
            > 0.3  → ×1.5 (увеличить)
            < -0.3 → ×0.7 (уменьшить)
        """
        # branching_factor
        if params_action[0] > 0.3:
            self.branching_factor = min(
                self.branching_factor + 1, MAX_BRANCHING_FACTOR
            )
        elif params_action[0] < -0.3:
            self.branching_factor = max(self.branching_factor - 1, 2)

        # max_depth
        if params_action[1] > 0.3:
            self.max_depth = min(self.max_depth + 1, MAX_DEPTH_HARD)
        elif params_action[1] < -0.3:
            self.max_depth = max(self.max_depth - 1, 2)
...
class OmegaLifeEnv(gym.Env):
    ...
    def __init__(
        ...
        super().__init__()
        if reward_mode not in {"order", "life", "growth"}:
            raise ValueError(
    ...
    def step(self, action):
        ...
        if self.action_mode == "scalar":
            ...
            self.engine.micro_action_vector(micro_vec)
            self.engine.apply_parameters_action(params_vec)
...
CONDITIONS = [
    ("frozen", "growth", "frozen", {"no_step": True}),
    ("random_params", "growth", "random", {}),
    ("trained_params", "growth", "trained", {}),
...
def main():
    print("=" * 72)
    print("ОМЕГА-ЭКСПЕРИМЕНТ (v7: RL управляет параметрами)")
    print("=" * 72)
    ...
    print("=" * 72)
    test_pairs = [
        ("trained_params", "random_params"),
        ("trained_params", "constant_reward"),
    ]
    test_rows = []
